...
const GoogleSignInButton = ({ 
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // Initialize Google Auth Service if needed
      ...
      if (googleResult.success) {
        // Send Google data to our backend
        const authResult = await authService.googleLogin(googleResult.data);
        
        if (authResult.success) {
          onSuccess && onSuccess(authResult.data);
        } else {
          onError && onError(authResult.message);
        }
      } else {
        onError && onError(googleResult.error);
      }
    } catch (error) {
